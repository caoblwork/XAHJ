...
<script type="text/javascript">
		...
		function callbackMultHandlerOrganUser(name,id){
			var namesVal = $("#jbox-iframe")[0].contentWindow.mergerStr(name,$('#users').val());
			var idsVal   = $("#jbox-iframe")[0].contentWindow.mergerStr(id,$('#userId').val());
			$('#groupName').val(namesVal);
			$('#linkManGroupId').val(idsVal);
    ...
    <form:form id="form" modelAttribute="voModel" method="post" action="save.do"
		...
		<div class="control-group">
			<label class="control-label" for="name">联系人名称:</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label" for="users">选择联系人组:</label>
			<div class="controls">
				<form:input readonly="true" path="groupName" type="text" class="span8" placeholder="点击选择" maxlength="12" check-type="required " required-message="请选择上级组!" />
            	<form:hidden path="linkManGroupId" />
            	<span class="add-on"><i
					class="icon-th"
					onclick="$.jBox.open('iframe:${ctx}/global/loadLinkManGroup.do?selectParent=true&group=false', '请单击选择联系人组', 400, 400, {iframeScrolling: 'no', buttons: {'清空': 0, '关闭': true},submit:cleanOrganUserData } )"></i></span>
			</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="cellPhone" type="text" class="span8" maxlength="11" />
