...
<script type="text/javascript">
    ...
    var parameter = {
        ...
        // 模块URL
        moduleUrl: '${ctx}/system/resource',
        //单个删除还是批量删除都走这个URL
        deletePageUrl:'${ctx}/system/resource/remove.do'
    };
    ...
    function deleteBtn(id){
        ...
        var confirm = function (v, h, f) {
            if (v == 'ok') {
                form.attr("action", '${ctx}/system/resource/delete.do?id='+id).submit();
                return true;
            }else if (v == 'cancel') {
                return true;
            }
...
<body>
    ...
    <form:form class="well form-inline" id="searchForm" modelAttribute="resourceSearchCondition" action="${ctx}/system/resource/list.do" method="post">
        <form:hidden id="pageNumber" path="pageNumber" />
        <form:hidden id="pageSize" path="pageSize" />
        <span>
            <label for="name">资源名称：</label>
            <form:input path="name" type="text" id="name" class="span3" maxlength="100" placeholder="资源名称"/>
        </span>   
        <span>
            <label for="type">资源类型：</label>
            <form:input path="type"  id="type" type="text"  Class="span3" maxlength="100" placeholder="资源类型"/>
        </span>
        <span>
