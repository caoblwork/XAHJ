...
public class ProcedureProcessController extends BaseProcedureController {
	...
	private void doProcessModify(String biz, ModelAndView mav, Map<String, String> beanMap) {
		...
		try {
			...
			BaseProcedureVo vo = (BaseProcedureVo) obj;
			vo.setProcessDefinitionId(processDefinition.getId());
			vo.setProcessDeployId(deployManager.getDeployId());
			String owner = (String) runtimeService.getVariable(task.getProcessInstanceId(), "owner");
			vo.setProcessInstanceId(task.getProcessInstanceId());
			vo.setProcessLaunchId(owner);
			if (!StringUtil.isValidString(vo.getId())) {
				BaseProcedureVo vo1 = (BaseProcedureVo)entityService.get(processInstance.getBusinessKey()).getResultObject();
				vo.setId(processInstance.getBusinessKey());
				vo.setProcessId(vo.getId());
				com.hangu.common.util.BeanUtils.beanShallowCopy(vo1, vo);
				entityService.save(vo1);	
			}else {
	...
	private void doProcessCreate(String biz, ModelAndView mav, Map<String, String> beanMap) {
		...
		try {
			...
			try {
				...
				processInstance = runtimeService.startProcessInstanceById(vo.getProcessDefinitionId(), bizKey);
				// 4.更新业务变量
				vo.setId(bizKey);
	...
	private void doProcessApproval(String biz, ModelAndView mav, Map<String, String> beanMap) {
		...
		try {
			...
			// 2.获取申请业务id
			String owner = (String) runtimeService.getVariable(task.getProcessInstanceId(), "owner");
			vo.setProcessId(processInstance.getBusinessKey());
			vo.setProcessInstanceId(task.getProcessInstanceId());
			vo.setProcessLaunchId(owner);
			entityService.save(vo);
			// 3.处理任务结点
